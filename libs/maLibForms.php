...
function mkForm($action="controleur.php",$method="get", $class = "")
{
...
function mkInput($type,$name,$value="", $misc=array(), $attrs="")
{
	...
	if (isset($misc["id"]) && isset($misc["label"]))
	{
		$id=$misc["id"];
		$label=$misc["label"];
		$positionLabel=$misc["positionLabel"];
		
		if ($positionLabel=="après")
		{
			echo "<input id=\"$id\" $attrs type=\"$type\" name=\"$name\" value=\"$value\"/>\n";
